...
data4 = bdg_loader.load_data(sys.argv[4]) #you can import any 4 bdg files through bdg_loader
fig, ax = plt.subplots(ncols=2,nrows=2)
ax[0,0].plot(data1['X'], data1['Y'])
ax[0,1].plot(data2['X'], data2['Y'])
ax[1,0].plot(data3['X'], data3['Y'])
ax[1,1].plot(data4['X'], data4['Y']) #plots the data
ax[0,0].fill_between(data1['X'], data1['Y'])
ax[0,1].fill_between(data2['X'], data2['Y'])
ax[1,0].fill_between(data3['X'], data3['Y'])
ax[1,1].fill_between(data4['X'], data4['Y']) #fills the bottom of the graphs
ax[0,0].set_title("H3k27ac DO")
ax[0,1].set_title("H3k27ac D2")
ax[1,0].set_title("Klf4")
ax[1,1].set_title("Sox2") # you could change this to "f{sys.argv[1, 2, 3, 4]}" instead of these defined titles
plt.tight_layout()

...
def compute_power2(tosses = numpy.array([10, 50, 100, 250, 500, 1000]), probs = numpy.around(numpy.arange(0.55, 1.05, 0.05), decimals=2)[::-1]): #define the matrix production function
...
matrix = compute_power2() #define the matrix to be equal to the power computation function

fig, ax = plt.subplots()
tosses = numpy.array([10, 50, 100, 250, 500, 1000])
probs = numpy.around(numpy.arange(0.55, 1.05, 0.05), decimals=2)[::-1]
ax = sns.heatmap(matrix, vmin = 0, vmax = 1, cmap = "viridis", xticklabels = tosses, yticklabels = probs) #calls seaborn within selected parameters and sets it to the axes we pulled from matplotlib
